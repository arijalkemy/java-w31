...
public class PrendaController {
    ...
    @GetMapping("/{code}")
    public ResponseEntity<Optional<Prenda>> findCode(@PathVariable Long code){
        return ResponseEntity.ok(service.findById(code));
    }

    @PutMapping("/{code}")
    public ResponseEntity<Prenda> UpdatePrenda(@PathVariable Long code, @RequestBody PrendaDTO prendaDTO){
        return ResponseEntity.ok(service.updatePrenda(code,prendaDTO));
    }

    @DeleteMapping("/{code}")
    public ResponseEntity<String> deletePrenda(@PathVariable Long code){
