...
public class VentaController {
    ...
    ///api/sale/{number}
    @GetMapping("/{id}")
    public ResponseEntity<Optional<Venta>> findById(@PathVariable Long id){
        return ResponseEntity.ok(service.findbyId(id));
    }
    @PutMapping("/{id}")
    public ResponseEntity<Venta> updateSale(@PathVariable Long id, VentaDTO ventaDTO){
    ...
    public ResponseEntity<List<Prenda>> prendasBySale(Long id){
