...
public class SaleController {
    ...
    @GetMapping("/{number}")
    public ResponseEntity<SaleDto> getSaleById(@PathVariable String number) {
        return new ResponseEntity<>(saleService.findById(number), HttpStatus.OK);
    }

    @PutMapping("/{number}")
    public ResponseEntity<SaleDto> updateSale(@PathVariable String number, @RequestBody SaleDto saleDto) {
        return new ResponseEntity<>(saleService.update(number, saleDto), HttpStatus.OK);
    }

    // Eliminar una venta en particular
    @DeleteMapping("/{number}")
    public ResponseEntity<Void> deleteSale(@PathVariable String number) {
    ...
    public ResponseEntity<List<ClothingItemDto>> getClothesBySaleNumber(@PathVariable String number) {
