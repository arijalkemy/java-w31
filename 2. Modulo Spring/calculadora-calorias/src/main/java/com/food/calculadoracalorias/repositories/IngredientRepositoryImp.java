package com.food.calculadoracalorias.repositories;

import com.fasterxml.jackson.core.exc.StreamReadException;
import com.fasterxml.jackson.core.type.TypeReference;
import com.fasterxml.jackson.databind.DatabindException;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.food.calculadoracalorias.dto.IngredientDTO;
import org.springframework.stereotype.Repository;
import org.springframework.util.ResourceUtils;

import java.io.File;
import java.io.FileNotFoundException;
import java.io.IOException;
import java.util.List;

@Repository
public class IngredientRepositoryImp implements IngredientRepository{
    private List<IngredientDTO> database;


    public IngredientRepositoryImp(){
        database = loadDataBase();
    }

    private List<IngredientDTO> loadDataBase() {
        File file = null;
        try{
            file = ResourceUtils.getFile("classpath:/Users/jcaparros/Downloads/calculadora-calorias/src/main/resources/food.json");
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }
        ObjectMapper objectMapper = new ObjectMapper();
        TypeReference<List<IngredientDTO>> typeRef = new TypeReference<>() {};
        List<IngredientDTO> priceDTOS = null;
        try{
            priceDTOS = objectMapper.readValue(file, typeRef);
        }catch (IOException e) {
            e.printStackTrace();
        }
        return priceDTOS;
    }

    @Override
    public IngredientDTO findIngredientByName(String name) {
        var first = database.stream().filter(x-> x.getName().equalsIgnoreCase(name)).findFirst();
        IngredientDTO result = null;

        if(first.isPresent()){
            result = first.get();
        }
        return result;
    }


}
