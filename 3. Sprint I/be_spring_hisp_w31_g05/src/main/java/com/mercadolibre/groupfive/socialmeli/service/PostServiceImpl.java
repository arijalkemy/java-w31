...
public class PostServiceImpl implements IPostService {
    ...
    public List<PostDto> findAllByPriceRange(Double min, Double max) {
      if (max < min) throw new BadRequestException("El rango de precios es incorrecto");
      List<Post> listOfPosts = this.postRepository.findAllByPriceRange(min, max);
      if(listOfPosts.isEmpty())throw new NotFoundException("No hay posts dentro del rango de precio establecido");
