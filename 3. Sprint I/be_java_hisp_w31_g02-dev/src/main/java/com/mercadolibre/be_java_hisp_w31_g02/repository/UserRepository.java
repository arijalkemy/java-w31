...
public class UserRepository implements IUserRepository{
    ...
    public Optional<User> findUserById(Integer idSeller) {
        return users.stream().filter(u -> u.getUserId() == idSeller).findFirst();
    }


    public Boolean existUser(Integer client){
        return users.stream().map(user -> user.getUserId()).toList().contains(client);
    }
    ...
    public List<User> findUsers(List<Integer> followedBy) {
        return users.stream().filter(u -> followedBy.contains(u.getUserId())).collect(Collectors.toList());
    }
