...
public class UserController {
    ...
    public ResponseEntity<UserListDTO> getSellerFollowers(@PathVariable Integer userId,
    ...
    public ResponseEntity<ResponseMessageDTO> follow(@PathVariable Integer userId, @PathVariable Integer userIdToFollow) {
        return ResponseEntity.ok(userService.follow(userId, userIdToFollow));
    }

    @PutMapping("/{userId}/unfollow/{userIdToUnfollow}")
    public ResponseEntity<ResponseMessageDTO> unfollow(@PathVariable Integer userId, @PathVariable Integer userIdToUnfollow) {
        return ResponseEntity.ok(userService.unfollow(userId, userIdToUnfollow));
    }

    @GetMapping("/{userId}/followed/list")
    ResponseEntity<UserListDTO> getSellersFollowed(@PathVariable Integer userId,
                                         @RequestParam(required = false) Optional<String> order){
    ...
    public ResponseEntity<List<ResponseFollowersCountDTO>> getRankingFollowersSeller(@RequestParam(required = false) String order){
