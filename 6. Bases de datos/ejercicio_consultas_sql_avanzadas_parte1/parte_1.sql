...
SELECT e.title, a.first_name, a.last_name FROM episodes e JOIN actor_episode ap ON e.id = ap.episode_id JOIN actors a ON ap.actor_id = a.id;

-- 3. Mostrar el título de todas las series y el total de temporadas que tiene cada una de ellas.

SELECT s.title, COUNT(se.id) AS total_seasons FROM series s JOIN seasons se ON s.id = se.serie_id GROUP BY s.title;

-- 4. Mostrar el nombre de todos los géneros y la cantidad total de películas por cada uno, siempre que sea mayor o igual a 3.

SELECT g.name, COUNT(m.id) AS total_movies FROM genres g JOIN movies m ON g.id = m.genre_id GROUP BY g.id, g.name HAVING COUNT(m.id) >= 3;
